	...
	public Boolean run(CommandSender sender, String[] args) {
		...
		if (args.length < 1 || !Util.isInteger(args[0])) {
			...
			if (id == null) {
				...
				Util.Message(
						Settings.getString(Setting.MESSAGE_USING_PREVIOUS_ID)
								.replace("<ID>", Integer.toString(id)), sender);
			}
		...
		int weekId = 0;
		try {
			PreparedStatement statement = IO
					.getConnection()
					.prepareStatement(
							"SELECT State, Player, Level, WeekID FROM weekly_completed WHERE ID = ?");
			...
			if (set.next()) {
				...
				if (state != 0) {
					Util.Message(
							Settings.getString(Setting.MESSAGE_DELETE_ONLY_OPEN_CHALLENGES),
							sender);
					set.close();
					statement.close();
					return true;
				...
				Util.Message(Settings.getString(Setting.MESSAGE_INVALID_ID),
						sender);
				set.close();
				statement.close();
				return true;
		...
		if (player != null) {
			if (message == null)
				Util.Message(
						Settings.getString(Setting.MESSAGE_SUBMISSION_REJECTED)
								.replace("<Level>", level), player);
			else
				Util.Message(
						Settings.getString(
								Setting.MESSAGE_SUBMISSION_REJECTED_MESSAGE)
		...
		try {
			PreparedStatement statement = IO
					.getConnection()
					.prepareStatement(
							"UPDATE weekly_completed SET state = 2, ModResponse=?, lastUpdate=? WHERE state <> 1 AND WeekID = ? AND Player = ?");
